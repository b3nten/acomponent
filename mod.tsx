...
type LooseObject = Record<string | number | symbol, any>
type Props = Record<string, string>

export function create_elm_component(name: string, component: {
	attrs?: string[],
	debug?: boolean,
	view: (props: {
		props: Props,
		state: LooseObject,
		dispatch: {
			[key: string]: (props?: LooseObject) => void
		}
	}) => ComponentChild,
	state?: (props: Props) => LooseObject,
	init?: ({
		props,
		state,
		dispatch
	...
	customElements.define(name,
		class extends HTMLElement {
			...
			private dispatcher(event: string, options: LooseObject) {
				if (!this.update) return;
				component.debug && console.log("dispatching", event, options)
				this.update({
					event,
					props: this.#getProps(),
					options,
			...
			private render() {
				component.debug && console.log("rendering", name)
				render(this.view({
					props: this.#getProps(),
					state: this.#state,
					dispatch: this.dispatch
			...
			public dispatch = new Proxy({}, {
				get: (target, prop) => {
					return (props: LooseObject) => {
						this.dispatcher(prop.toString(), props)
					}
				}
			}) as {
				[key: string]: (props?: LooseObject) => void
			...
			connectedCallback() {
				component.debug && console.log("connected", name)
				this.#state = this.state ? this.state(this.#getProps()) : {};
				this.init && this.init({
					props: this.#getProps(),
					state: this.#state,
					dispatch: this.dispatch
			...
			attributeChangedCallback(a,b,c) {
				this.dispatch.attribute_change({
					name: a,
					old: b,
					new: c
				})
